...
short int selectMode(unsigned short int debugMode) {
	...
	if (newMode < 0 || newMode > MODE_LENGTH) {
		fprintf(stderr,
				"ATTENZIONE. La modalita' scelta ( %d ) non e' valida.\n",
				newMode);
...
int importProcesses(char *buffer, struct processesData processes,
	...
	for (counter = 0;
		...
		nTokens = sscanf(next, "%s %u %u\n", (processes.processes[counter]),
				&(processes.arrivals[counter]),
				&(processes.durations[counter]));
		if (nTokens != MAX_TOKENS) {
			fprintf(stderr,
					"%d token rilevato da sscanf. Si aspetta che siano %d token per ogni riga.\n",
					nTokens, MAX_TOKENS);
		...
		if (!processes.durations[counter]) {
			fprintf(stderr,
					"ERRORE. Rilevato processo con durata nulla. Il programma verra' chiuso.\n");
			return counter + sizeof(char);
		}
		++next;
	}
